 ...
 * @internal
 */
export type TransformersMapping =
  | Record<string, AnyDataTransformer>
  | MaybeArray<AnyDataTransformer>
...
export interface ResolvedTransformer extends ResolvedDataTransformer {
  /**
   * A reference to the original transformer object is required for a comparison.
   */
  original: AnyDataTransformer
}
...
export function resolveTransformers(options?: TransformersMapping): ResolvedTransformer[] {
  ...
  if (
    ...
    const original = options as ResolvedDataTransformer
  ...
  if (
    ...
    const transformer: ResolvedDataTransformer = { id, input: original, output: original }
