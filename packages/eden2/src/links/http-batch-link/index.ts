...
>(options: TConfig = {} as any): HttpBatchLinkResult<TElysia, TConfig['key']> {
  ...
  const edenParamsResolver = async (op: Operation<TElysia, TConfig['key']>) => {
    return await resolveHttpOperationParams(options, op)
  }
  ...
  > = {
    ...
    async fetch(batchOps) {
      ...
      // For some reason, this type is not inferred...
      const params: EdenRequestParams<TElysia, TConfig['key']>[] = resolvedBatchOps

      const resolvedBatchParams = await serializer(params)
