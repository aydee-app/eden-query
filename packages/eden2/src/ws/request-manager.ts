...
type MessageIdLike = EdenRequestId

export interface EdenStoppedSubscriptionMessage extends EdenBaseEnvelope {
  method: 'subscription.stop'
}

export interface EdenProcedureMessage extends EdenBaseEnvelope {
  method: Procedure
  params: EdenProcedureRequest
}

export interface EdenReconnectRequest extends EdenBaseEnvelope {
  method: 'reconnect'
}

export interface WebSocketOperationResult<TOutput, TError> {
  id: EdenRequestId
  result: OperationResult<TOutput, TError>
}

export type EdenIncomingMessage<TOutput = unknown, TError = EdenFetchError> =
  | EdenReconnectRequest
  | WebSocketOperationResult<TOutput, TError>

export type EdenClientOutgoingMessage = EdenProcedureMessage | EdenStoppedSubscriptionMessage

export type WebSocketRequestCallbacks = Observer<EdenIncomingMessage, EdenClientError<AnyElysia>>
...
interface WebSocketRequest {
  message: EdenClientOutgoingMessage
  end: Promise<void>
...
export class RequestManager {
   ...
   */
  private outgoingRequests = new Array<WebSocketRequest & { id: MessageId }>()
   ...
   */
  private pendingRequests: Record<MessageId, WebSocketRequest> = {}
   ...
   */
  public register(message: EdenClientOutgoingMessage, callbacks: WebSocketRequestCallbacks) {
