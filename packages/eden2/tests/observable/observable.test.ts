...
import { Observable, observableToAsyncIterable, share, tap } from '../../src/observable'

describe('observable', () => {
  test('vanilla observable - complete()', () => {
    const obs = new Observable<number, Error>((observer) => {
      observer.next(1)
      observer.complete()
    })
  ...
  test('observableToAsyncIterable()', async () => {
    ...
    for await (const value of observableToAsyncIterable(obs, new AbortController().signal)) {
