...
The Eden-Query project is a collection of libraries that includes the RPC client, [`@aydee-app/eden`](/eden),
and framework-specific integrations, such as [`@aydee-app/eden-react-query`](/eden-query/react) and
[`@aydee-app/eden-svelte-query`](/eden-query/svelte) for React and Svelte respectively.

> [!NOTE]
> The [`@aydee-app/eden`](/eden) library is a distinct, but functionally identical implementation of the
...
See the [`examples` directory](https://github.com/aydee-app/eden-query/tree/main/examples) for examples.
...
```typescript twoslash
import { createEdenTreatyReactQuery, httpLink } from '@aydee-app/eden-react-query'
...
// @filename: index.ts
// ---cut---
import { createEdenTreatyReactQuery, httpBatchLink } from '@aydee-app/eden-react-query'
import type { App } from './server'
