...
<article class="mx-auto w-full max-w-5xl space-y-4 p-4">
  <h1 class="text-center text-2xl font-medium text-gray-500 dark:text-gray-400">
    <p
      class={cn(
        inView.current ? 'animate-in' : 'animate-out',
        'fade-out fade-in slide-in-from-top-4 fill-mode-both duration-1000 ease-in-out',
        'bg-gradient-to-r from-rose-400 to-amber-400 bg-clip-text text-center text-7xl leading-[6rem] font-semibold text-transparent',
      )}
    >
      Community
    </p>
  ...
  >
    {#each columns as column}
      <li class={cn(column % 2 ? 'py-0' : 'lg:py-12', 'flex flex-col gap-4')}>
        {#each tweets.filter((_, index) => index % columns.length === column) as tweet}
          <a
            href={tweet.href}
            class="btn text-transform-none hover:border-neutral-content transition:transform h-fit justify-start gap-4 rounded-xl border p-4 text-left font-normal hover:-translate-y-1"
          >
            <div class="flex items-center gap-4">
              <img loading="lazy" class="h-12 w-12 rounded-full" src={tweet.image} alt="Tweet" />

              <div>
                <p class="font-bold text-gray-700 dark:text-gray-300">{tweet.user}</p>
                <p class="text-sm text-gray-500 dark:text-gray-400">@{tweet.id}</p>
              </div>
            </div>

            <p class="text-base whitespace-pre-line">{tweet.content}</p>
          </a>
        {/each}
      </li>
    {/each}
