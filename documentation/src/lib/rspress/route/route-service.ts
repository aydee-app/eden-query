...
import { getRoutePathParts, normalizeRoutePath } from './normalize-route-path'
...
export class RouteService {
  ...
  addRoute(routeInfo: RouteMeta) {
    ...
    if (this.routeData.has(routePath)) {
      ...
      const currRouteExtIndex = this.#extensions.indexOf(path.extname(absolutePath))
  ...
  generateRoutesCodeByRouteMeta(routeMeta: RouteMeta[], isStaticImport: boolean) {
    return `
